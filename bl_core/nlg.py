    ...
    def _generate_attachment(self, content):
        ...
        for e in content['elements']:
            ...
            if '$' in e['output']['elements']['value']:
                val_lookup = e['output']['elements']['value'].replace(
                    "$", "").split(".")
            ...
            if '$' in e['output']['elements']['text']:
                text_lookup = e['output']['elements']['text'].replace(
                    "$", "").split(".")
                if isinstance(data, list):
                    for d in data:
                        t = None
            ...
            if '$' in e['output']['elements']['sub_text']:
                subtext_lookup = e['output']['elements']['sub_text'].replace(
                    "$", "").split(".")
                if isinstance(data, list):
                    for d in data:
                        st = None
            ...
            if '$' in e['output']['elements']['image']:
                img_lookup = e['output']['elements']['image'].replace(
                    "$", "").split(".")
                if isinstance(data, list):
                    for d in data:
                        i = None
        ...
        for x in range(len(labels)):
            ...
            btn_pack.append(tmp)
        result = {}
        default_answer = e['output']['elements'].get(
            'default_response_text', '')
        result["elements"], result['type'] = self._generate_by_type(
            e['output']['type'], postbacks, labels, subtitle, images, btn_pack, e['output']['elements']['title'], embed, split_text, default_answer)
        return result
